...
export default function PlayerStatistics({ stats }: { stats: PlayerStats[] }) {
  ...
  const bestScorers = useMemo(() => {
    return [...stats]
      .sort(
        (a, b) =>
          normalizeValue("goals", b.goals) - normalizeValue("goals", a.goals)
      )
      .slice(0, 3);
  }, [stats]);
  ...
  const bestPassers = useMemo(() => {
    return [...stats]
      .sort(
        (a, b) =>
          normalizeValue("assists", b.assists) -
          normalizeValue("assists", a.assists)
      )
      .slice(0, 3);
  }, [stats]);
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-4">
      <div className="flex items-center gap-10 justify-center font-Montserrat">
        <p
          onClick={() => setAllStatsWindow("galerie")}
        ...
        <p
          onClick={() => setAllStatsWindow("allStats")}
          className={`cursor-pointer ${
            allStatsWindow === "allStats"
              ? "border-b-2 border-aja-blue font-semibold"
              : ""
          }`}
        >
      ...
      {allStatsWindow === "galerie" ? (
        <>
          <p className="text-center sm:text-left font-Montserrat font-semibold text-lg">
            Meilleurs buteurs
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-10">
            {bestScorers.map((player, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-white p-4 rounded-md flex flex-col items-center gap-2"
                >
                  <Image
                  ...
                  />
                  <p className="text-xl sm:text-2xl font-semibold font-Bai_Jamjuree">
                    {player.nom}
                  </p>
                  <p className="font-Montserrat text-xs">
                    {player?.position}, {info?.age} ans
                  </p>
                  <div className="text-center font-Montserrat text-sm">
          ...
          <table className="mx-auto bg-white max-w-[1200px]">
            <thead className="font-Montserrat text-sm">
              <tr>
                ...
                >
                  Effectif{" "}
                  {sortKey === "matches" && (sortOrder === "asc" ? "↑" : "↓")}
                </th>
                <th
                  className="border border-gray-300 px-4 py-2 cursor-pointer"
                  onClick={() => handleSort("titularisations")}
                >
                  Matches{" "}
                  {sortKey === "titularisations" &&
                    (sortOrder === "asc" ? "↑" : "↓")}
                </th>
                <th
                  className="border border-gray-300 px-4 py-2 cursor-pointer"
                ...
                >
                  Rouges{" "}
                  {sortKey === "red_cards" && (sortOrder === "asc" ? "↑" : "↓")}
                </th>
                <th
                  className="border border-gray-300 px-4 py-2 cursor-pointer"
                  onClick={() => handleSort("substitutions_in")}
                >
                  Entrées{" "}
                  {sortKey === "substitutions_in" &&
                    (sortOrder === "asc" ? "↑" : "↓")}
                </th>
                <th
                  className="border border-gray-300 px-4 py-2 cursor-pointer"
                  ...
                  <tr key={index}>
                    ...
                    <td className="border border-gray-300 px-4 py-2">
                      {player.titularisations ===
                        "Pas dans l'effectif cette saison" ||
                      player.titularisations ===
                        "N'a pas été utilisé cette saison"
                        ? 0
                        : player.titularisations}
                    </td>
                    <td className="border border-gray-300 px-4 py-2">
